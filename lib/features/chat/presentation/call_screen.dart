...
class _CallScreenState extends State<CallScreen> {
  ...
  void _startSimulatedFallback() {
    debugPrint('[CallScreen] Starting simulated audio/video call simulation...');
    setState(() {
      _callStatusText = widget.isCaller ? 'Dialing (Simulated)...' : 'Connecting (Simulated)...';
    });
  ...
  void _listenToSignaling() {
    ...
    _signalingSubscription = SupabaseService.streamCallSession(_currentSessionId!).listen((event) async {
      if (event.isEmpty || !mounted) return;

      final session = CallSessionModel.fromJson(event);

      // Handle Call Ending/Rejection
      if (session.status == CallStatus.rejected || session.status == CallStatus.ended) {
      ...
      if (widget.isCaller && session.sdpAnswer != null && _peerConnection != null) {
      ...
      if (!widget.isCaller && session.sdpOffer != null && _peerConnection != null) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (isVideo && _remoteRenderer.srcObject != null && _callStatusText == 'Connected')
            Positioned.fill(
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: const BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: Colors.redAccent, blurRadius: 10, spreadRadius: 1)],
                        ),
